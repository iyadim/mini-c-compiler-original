...
class CodeGenVisitor(Visitor):
    ...
    def vTranslationUnit(self, node):
        ...
        self.o("# Last Author: Iyad Milhem(Spring 2022)\n")
        self.o("# Original Author: Atul Varma (Spring 2004)\n")
        self.o("    .text")
        # add the StackGuard canary (32bit) value for simplicity we are using constant
        """modified"""
        """Declare the stack guard canary value and stack guard fail function"""
        self.o(" # DECLARING  STACK GUARD VALUE CONSTANT FOR THIS PROJECT")
        self.o(" #")
        self.o("    .globl	___stack_chk_guard")
        self.o("    .data")
        self.o("    ___stack_chk_guard:")
        self.o("      .int 0xDEADBEEFu")
        # add  stack guard error callback function
        self.o("STACKGUARD0:")
        self.o(" 	.ascii \"ERROR: __stack_chk_guard has been corrupted\"")
        self.o("    .text")
        self.o(" 	.globl	___stack_chk_fail")
        self.o("    .def	___stack_chk_fail;	.scl	2;	.type	32;	.endef")
        self.c("BEGIN FUNCTION: stack_chk_fail()")
        self.o("___stack_chk_fail:")
        self.o("	pushl	%ebp")
        self.o("    movl	%esp, %ebp")
        self.o("	subl	$24, %esp")
        self.o("	movl	$LC0, (%esp)")
        self.o("	call	_puts")
        self.o(" 	nop")
        self.o("    pushl $0                          # Push arg 1")
        self.o("    call _exit                        # Call exit()")
        self.o("    addl $4, %esp                     # Deallocate argument stack")
        self.o("	leave")
        self.o("	ret")
        self.o("# End FUNCTION: stack_chk_fail()")
    ...
    # modified
    # modify the return to check stack guard canary
    def vReturnStatement(self, node):
            done_label = self.curr_func_end_label.split("_")[0] + "_done"
            self.o("\n# Compare stack guard Canary \n")
            self.o("  movl -4(%ebp), %eax", "Get canary value from stack")
            self.o("  xorl ___stack_chk_guard, %eax", "Compare %eax canary with original canary")
            self.o("  je %s" % done_label)
            self.o("  call ___stack_chk_fail", "It is not same canary go to fail function ")
            self.c("If the canary is the same there is no buffer overflow and it will go to next step.")
            self.o("%s:" % done_label)
            return_reg = self._accept_and_pop(node.expr)
            self.o("  movl %s, %%eax" % return_reg, "Set return value")
            self.o("  jmp %s" % self.curr_func_end_label, "Exit function")
